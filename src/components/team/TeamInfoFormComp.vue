    ...
    <v-container>
        <v-form ref="form" lazy-validation>
            ...
            <v-row>
                <v-data-table
                :headers="headers"
                :items="teamInfo.teamRegularExerciseList"
                hide-default-footer>
                    <template v-slot:top>
                        <v-toolbar flat>
                            ...
                            <v-spacer/>
                            <v-btn @click="createExerciseTime()" fab dark small color="primary"><v-icon dark>mdi-plus</v-icon></v-btn>
                        </v-toolbar>
                    </template>
                    
                    <template v-slot:item="{ item, index }">
                        <tr :data-id="index">
                            <td><v-autocomplete :items="days" dense v-model="item.dayOfTheWeekCode"/></td>
                            <td><v-autocomplete :items="times" dense v-model="item.startTime"/></td>
                            <td><v-autocomplete :items="times" dense v-model="item.endTime"/></td>
                            <td><v-text-field disabled v-model="item.exercisePlaceAddress"/></td>
                            <td><v-btn @click="addressAPI(index)" color="secondary" x-small fab dark><v-icon>mdi-pencil</v-icon></v-btn></td>
                            <td><v-text-field v-model="item.exercisePlaceName"/></td>
                            <td><v-btn @click="deleteExerciseTime(index)" color="secondary" x-small dark fab><v-icon>mdi-delete</v-icon></v-btn></td>
                        </tr>
                    </template>
                </v-data-table>
            </v-row>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                { text: '요일'      , value: 'dayOfTheWeekCode', sortable: false, width: '10%', align: 'start', },
    ...
    methods : {
        ...
        addressAPI(idx) {
            new window.daum.Postcode({
                oncomplete: (data) => {
                    this.teamInfo.teamRegularExerciseList[idx].exercisePlaceAddress = data.address;
                }
            }).open();
        },
        hometownAPI() {
        ...
        createExerciseTime() {
            this.teamInfo.teamRegularExerciseList.push({});
        },
        deleteExerciseTime(idx) {
            const isRemainOneExercise = this.teamInfo.teamRegularExerciseList.length == 1;
            if (isRemainOneExercise) {
                alert("정기 운동시간 입력란을 삭제할 수 없습니다.");
                return;
            }
            this.teamInfo.teamRegularExerciseList.splice(idx, 1);
        },
    }
}
</script>
